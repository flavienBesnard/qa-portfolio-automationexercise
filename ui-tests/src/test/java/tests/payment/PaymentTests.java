...
public class PaymentTests extends BaseTest {
    /**
     * CT-ID : CT-PAYMENT-001
     * EX-ID : EX-20
     *
     * Objectif : Paiement réussi et confirmation de commande affiché
     * Critères de réussite : - Un message de succès de type "Congratulations! Your order has been confirmed!" est affiché
     ...
     *
     * Note : Selenium n'encourage pas à télécharger les fichiers, l'approche recommandé est de valider le lien côté UI puis via HTTP plutôt que WebDriver
     */

    @Test(groups = {"ui","payment","regression"}, description = "CT-PAYMENT-001 / EX-20 / Paiement réussi et confirmation de commande affiché")
    public void should_payment_work_and_display_confirmation() throws Exception  {
        ...
        String cookieHeader = driver().manage().getCookies().stream().map(c -> c.getName() + "=" + c.getValue()).collect(Collectors.joining("; "));
        ...
        HttpRequest req = HttpRequest.newBuilder(invoiceUri).header("Cookie", cookieHeader).GET().build();
